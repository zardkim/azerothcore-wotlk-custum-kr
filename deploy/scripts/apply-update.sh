...
if [ "$ASSUME_YES" -ne 1 ]; then
  echo "########################################################################"
  echo "  코어/모듈 업데이트 적용"
  echo "  1) 계정/캐릭터 백업  2) 새 이미지 pull  3) DB에 증분 SQL 반영(dbimport)"
  echo "  4) worldserver/authserver를 새 이미지로 교체 재기동"
...
echo ">> [1/4] 업데이트 적용 전 계정/캐릭터 백업 중..."
./scripts/backup.sh

echo ">> [2/4] 새 이미지 pull 중..."
docker compose pull

echo ">> [3/4] DB에 증분 SQL 반영 중 (ac-db-import)..."
docker compose up --exit-code-from ac-db-import ac-db-import

echo ">> [4/4] worldserver/authserver 등을 새 이미지로 교체 재기동..."
docker compose up -d
